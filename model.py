...
class Critic(nn.Module):
    ...
    def forward(self, state, action):
        ...
        x = torch.cat((xs, action), dim=1)  # We would like to concatenate the actions with a richer (feature) representation of the input, so actions are concatenated after first layer. 
        x = F.relu(self.fc2(x))
        return self.fc3(x)